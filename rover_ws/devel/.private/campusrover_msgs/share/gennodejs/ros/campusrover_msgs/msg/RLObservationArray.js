...
class RLObservationArray {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.observation.forEach((val) => {
      bufferOffset = RLObservation.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new RLObservationArray(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.observation[i] = RLObservation.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.observation.forEach((val) => {
      length += RLObservation.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new RLObservationArray(null);
    ...
    if (msg.observation !== undefined) {
      ...
      for (let i = 0; i < resolved.observation.length; ++i) {
        resolved.observation[i] = RLObservation.Resolve(msg.observation[i]);
      }
    }
    else {
...
module.exports = RLObservationArray;
