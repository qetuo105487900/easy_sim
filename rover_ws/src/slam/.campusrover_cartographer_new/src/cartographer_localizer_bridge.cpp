...
bool CartographerLocalizerBridge::loadMapService(campusrover_msgs::MapLoadInfo::Request &req, campusrover_msgs::MapLoadInfo::Response &res)
{
...
void CartographerLocalizerBridge::initPoseCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &msg)
{
  // Finish trajectory
...
void CartographerLocalizerBridge::submapCallback(const cartographer_ros_msgs::SubmapList::ConstPtr &msg)
{
...
bool CartographerLocalizerBridge::enLocalizerService(campusrover_msgs::LocalizerSwitch::Request &req, campusrover_msgs::LocalizerSwitch::Response &res)
{
  ...
  else if(req.mode ==  campusrover_msgs::LocalizerSwitch::Request::MODE_ENABLE)
  {
    ...
    if (!carto_cr_start_trajectory_.call(cft_start_srv)) 
    {
      ...
      res.message = "Failed to start new trajectory with id " + std::to_string(new_traj_id);
      return false;
    }
    std::cout << "[Carto Bridge] Starting trajectory success" << std::endl;
    carto_map_state_.trajectory_robot = new_traj_id;
    res.message = "Success to start new trajectory with id " + std::to_string(new_traj_id);
    return true;
  }
...
bool CartographerLocalizerBridge::deleteMapService(std_srvs::Empty::Request &req, std_srvs::Empty::Response &res)
{
...
void CartographerLocalizerBridge::updateTransform( const geometry_msgs::TransformStamped tf)
{
...
void CartographerLocalizerBridge::updateBaseMap(CgMapState &state, const std::string building, const std::string floor)
{
  cartographer_ros_msgs::FinishTrajectory cft_finish_srv;
  ...
  if (carto_finish_trajectory_.call(cft_finish_srv))
  {
    ...
    if (!state.map_exist)
    {
      ...
      cft_loadmap_srv.request.filename = map_folder_+building+"/"+prifixB+floor_str+prifixF+".pbstream";
...
CartographerLocalizerBridge::CartographerLocalizerBridge(/* args */)
{
  ...
  cr_enable_localizer_srv_ = n_.advertiseService("enable_localizer", &CartographerLocalizerBridge::enLocalizerService, this);
  cr_delete_map_srv_ = n_.advertiseService("delete_map", &CartographerLocalizerBridge::deleteMapService, this);
  cr_load_map_srv_ = n_.advertiseService("load_map", &CartographerLocalizerBridge::loadMapService, this);
}
