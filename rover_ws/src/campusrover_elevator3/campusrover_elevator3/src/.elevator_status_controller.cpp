...
ros::Subscriber elevator_caller_sub_,door_status_sub_,floor_status_sub_,elevator_info_sub_,elevator_pose_sub_,table_pose_sub_,mpc_finish_sub_, panel_pose_sub_, button_detection_sub_;
...
void TimerCallback(const ros::TimerEvent &event)
{
  ...
  else if(control_status_ == status.ENTER_ELEVATOR_INSIDE_NO_ROTATE)//enter the elevator inside
  {
    ...
    if (elevator_pose_.poses.at(elevator_id).position.z == -1)
    {
      ...
      {
        table_position_msg.request.position.y = 0.4;
        table_position_msg.request.position.z = 0.5;
        ArmPositionFunctionCallService(arm_table_position_srv_client_, table_position_msg);
        // press l
        std_msgs::Int16 msg;
      ...
      {
        table_position_msg.request.position.y = 0.33;
        table_position_msg.request.position.z = 0.5;
        ArmPositionFunctionCallService(arm_table_position_srv_client_, table_position_msg);
        // press c
        std_msgs::Int16 msg;
  ...
  else if(control_status_ == status.MOVE_TO_INSIDE_BUTTON_NO_ROTATE)//move to front of button (inside)
  {
    ...
    if( (elevator_caller_status_ == 31 || elevator_caller_status_ == 32)  &&  status_time.frame_id != "[inside]Button + VS_NO_ROTATE")
    {
  ...
  else if(control_status_ == status.ENTER_ELEVATOR_INSIDE_ROTATE)//enter the elevator inside
  {
